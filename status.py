...
def main():
    ...
    total = len(ASSETS)
    ncols = max(1, min(total, 4))
    nrows = max(1, (total + ncols - 1) // ncols)
    ...
    for ax, a in zip(axes_flat, ASSETS):
        ...
        if times:
            ...
            floors.append(
                floors[-1] if floors else (avg_entry * 0.95 if avg_entry else live_price * 0.95)
            )
        ...
        if floors:
            ax.step(
                times, floors, where="post", color="#e74c3c",
        ...
        plt.setp(
            ax.xaxis.get_majorticklabels(), rotation=30, ha="right",
        ...
        ax.text(
            ...
            bbox=dict(facecolor="#1a1d27", edgecolor=pl_color, boxstyle="round,pad=0.3", alpha=0.9),
        )
    ...
    total_pl = sum(float(p.unrealized_pl) for p in trading.get_all_positions())
    ...
    fig.text(
        ...
        bbox=dict(facecolor="#0f1117", edgecolor="#333", boxstyle="round,pad=0.4"),
    )

    fig.suptitle("Trading Bot — Live Status", color="white", fontsize=13, fontweight="bold", y=0.99)
